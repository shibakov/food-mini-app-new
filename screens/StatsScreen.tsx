...
// View-model for stats chart, mapped from backend StatsResponse
interface StatsDayViewModel {
  kcal: number;
  p: number;
  f: number;
  c: number;
...
const StatsScreen: React.FC<StatsScreenProps> = ({ onSettingsClick, isOffline }) => {
  ...
  useEffect(() => {
    ...
    api.stats
      .get(rangeMap[period])
      .then(res => {
        // Backend returns StatsResponse { range, items: StatsDay[] }
        const items = Array.isArray(res.items) ? res.items : [];
        const mapped: StatsDayViewModel[] = items.map(d => ({
          kcal: d.calories,
          p: d.protein,
          f: d.fat,
          c: d.carbs,
          // Use day-of-month as a simple label for now
  ...
  const avgKcal = data.length
    ? Math.round(data.reduce((acc: number, c: StatsDayViewModel) => acc + c.kcal, 0) / data.length)
    : 0;
  const avgProtein = data.length
    ? Math.round(data.reduce((acc: number, c: StatsDayViewModel) => acc + c.p, 0) / data.length)
    : 0;
  const avgFats = data.length
    ? Math.round(data.reduce((acc: number, c: StatsDayViewModel) => acc + c.f, 0) / data.length)
    : 0;
  const avgCarbs = data.length
    ? Math.round(data.reduce((acc: number, c: StatsDayViewModel) => acc + c.c, 0) / data.length)
    : 0;
          ...
          <>
            <Card variant="regular" className="pt-6 pb-6">
              <div className="flex justify-between items-baseline px-1 mb-6">
                ...
                <span className="text-xs font-semibold text-gray-400">
                  Avg: <span className="text-gray-900">{avgKcal}</span>
                </span>
              </div>
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col justify-end h-full group">
                        <div className="w-full relative flex items-end bg-transparent rounded-t-sm overflow-hidden h-full">
                          <div
                            className={`w-full rounded-t-[3px] transition-all duration-500 ${getBarColor(
                              day.kcal
            ...
            <Card variant="regular">
              ...
              <div className="space-y-6">
                {[
                  { label: 'Protein', avg: avgProtein, data: data.map((d: StatsDayViewModel) => d.p), max: 200, color: 'bg-blue-500' },
                  { label: 'Fats', avg: avgFats, data: data.map((d: StatsDayViewModel) => d.f), max: 100, color: 'bg-amber-400' },
                  { label: 'Carbs', avg: avgCarbs, data: data.map((d: StatsDayViewModel) => d.c), max: 300, color: 'bg-orange-400' },
                ].map((macro, idx) => (
                  <div key={idx} className="flex flex-col gap-2">
                    <div className="flex justify между items-baseline px-1">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{macro.label}</span>
                      <span className="text-xs font-bold text-gray-900">
                        {macro.avg}g <span className="text-gray-300 font-normal">avg</span>
                      </span>
                    </div>
